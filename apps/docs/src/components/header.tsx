...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b bg-fd-background/95 backdrop-blur supports-[backdrop-filter]:bg-fd-background/60">
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                {link.label}
                {isActive && (
                  <span className="absolute inset-x-1 -bottom-[17px] h-0.5 rounded-full bg-fd-primary" />
                )}
              </Link>
            );
          })}
